function renderTeacher(app) {
    ...
    return `
      <div class="card">
        ...
        ${loading
          ...
          : showRegistry ? renderExamsTable(filtered) : ''
        }
      </div>
    `;
  }
  ...
  function renderExamsTable(filtered) {
    if (filtered.length === 0) {
      return `<p class="text-center text-gray" style="padding:2rem">
  ...
  function bindListaEvents() {
    ...
    document.querySelectorAll('[data-edit]').forEach(btn => {
      btn.onclick = () => {
        const exam = exams.find(x => x.id === btn.dataset.edit);
    ...
    document.querySelectorAll('[data-del-exam]').forEach(btn => {
      btn.onclick = () => {
        const exam = exams.find(x => x.id === btn.dataset.delExam);
        if (exam) deleteExam(exam);
      };
    });
  }
